...
def optimize_mc(N,A0):  
    ...
    def model_rss(param):
        ...
        rss, S = fit_data(exp_data, rutherford_opt(N,A), S0)
        print("RSS " + str(rss))
        print("S " + str(S))
        return rss
    
    tol = (A_max - A_min)/1000
    print(tol)
    
    result = noisyopt.minimizeCompass(model_rss, x0=[A0], bounds=[(A_min,A_max)], method='Nelder-Mead', deltatol = (A_max - A_min)/10, paired=False, errorcontrol=True, disp=True)
    A_best = result.x[0]
    print(result)

    data_best = rutherford_opt(N*50,A_best)
    csv_name = 'rutherford_mc_fast_opt_lowconf ' + str(N*50) +str(A_best)+ '.csv'
    np.savetxt(csv_name, data_best.transpose(), delimiter=',',fmt='%.10e')
    
    print("Saved " + 'final  low confident' + str(N*50) +str(A_best)+ '.csv')
